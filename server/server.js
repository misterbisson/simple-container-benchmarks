...
function disk(req, res, next) {
	...
	var thing = exec("(dd bs=1M count=1024 if=/dev/zero of=/benchmarks/simple-container-benchmarks-writetest conv=fdatasync) 2>&1 | tail -1 | sed -e 's/^ *//' -e 's/ *$//'", {silent:true}).output;
